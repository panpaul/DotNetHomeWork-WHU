...
namespace OrderView
{
    public partial class Modify : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            var service = form1?.OrderService;
            if (isModify)
            {
                service?.Where(o => o.OrderId == order.OrderId).Update(order.Customer);
                service?.Where(o => o.OrderId == order.OrderId).Update(order.Products);
            }
            else
            {
                service?.Create(order, out _);
            }
